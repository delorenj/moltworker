...
export function buildEnvVars(env: OpenClawEnv): Record<string, string> {
  ...
  if (env.TAILSCALE_AUTHKEY) envVars.TAILSCALE_AUTHKEY = env.TAILSCALE_AUTHKEY;
  if (env.TAILSCALE_HOSTNAME) envVars.TAILSCALE_HOSTNAME = env.TAILSCALE_HOSTNAME;
  if (env.TAILSCALE_STATE_DIR) envVars.TAILSCALE_STATE_DIR = env.TAILSCALE_STATE_DIR;
  if (env.TAILSCALE_SERVE_TARGET) envVars.TAILSCALE_SERVE_TARGET = env.TAILSCALE_SERVE_TARGET;
